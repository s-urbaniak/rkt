...
func appUnit(p *stage1commontypes.Pod, ra *schema.RuntimeApp, interactive bool, flavor string, privateUsers string, insecureOptions Stage1InsecureOptions) error {
	...
	w := newUnitWriter(p)
	w.writeUnit(ServiceUnitPath(p.Root, appName), "failed to create service unit file", opts...)
	w.activate(ServiceUnitName(appName), ServiceWantPath(p.Root, appName))

	w.writeUnit(
		ServiceUnitPath(p.Root, types.ACName(fmt.Sprintf("reaper-%s", appName))),
...
// unitWriter is the type that writes systemd units preserving previous errors.
// Any write* receiver method can be invoked multiple times without error checking.
// If a previous invocation generated an error, the write* operation will be skipped.
// If any error occured during write* invocations, it can be retrieved using error().
...
func (u *unitWriter) writeUnit(path string, errmsg string, opts ...*unit.UnitOption) {
	if u.err != nil {
		return
	}
	...
	if err != nil {
		u.err = errwrap.Wrap(errors.New(errmsg), err)
		return
	}
	defer file.Close()
...
func (u *unitWriter) writeShutdownService(exec string, opts ...*unit.UnitOption) {
	if u.err != nil {
		return
	}

	flavor, systemdVersion, err := GetFlavor(u.p)
	if err != nil {
		u.err = errwrap.Wrap(errors.New("failed to create shutdown service"), err)
		return
	}
	...
	u.writeUnit(
		ServiceUnitPath(u.p.Root, "shutdown"),
...
func (u *unitWriter) activate(unit, wantPath string) {
	if u.err != nil {
		return
	}

	if err := os.Symlink(path.Join("..", unit), wantPath); err != nil {
		u.err = errwrap.Wrap(errors.New("failed to link service want"), err)
	}
}

// error returns the first error that occured during write* invocations.
func (u *unitWriter) error() error {
	return u.err
}
